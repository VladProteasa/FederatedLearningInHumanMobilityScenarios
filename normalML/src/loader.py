    ...
    def read_data(self):
        ...
        for i, line in enumerate(tqdm.tqdm(entries, desc="read data")):
            ...
            if prev_user == user and (
                ...
                # and (abs(prev_lat - lat) <= 0.1 and abs(prev_lon - lon) <= 0.1) 
                # and not checkins >= self.min_checkins
            ):
                ...
                if checkins >= self.min_checkins:
                    ...
                    #     break
                    split_validation = int(len(user_time) * 0.7)
                    split_test = int(len(user_time) * 0.85)
                    ...
                    test_times.append(user_time[split_test:])
                    test_lats.append(user_lat[split_test:])
                    test_lons.append(user_lon[split_test:])
                    test_delta_lats.append(user_delta_lats[split_test:])
                    test_delta_lons.append(user_delta_lons[split_test:])
                    test_locs.append(user_loc[split_test:])
        ...
        #     map(
        #         lambda x: [
        #             float(y - mean_lat) / deviation_lat for y in x
        #         ],
        #         delta_lats,
        #     )
        # )
        ...
        #     map(
        #         lambda x: [
        #             float(y - mean_lon) / deviation_lon for y in x
        #         ],
        #         delta_lons,
        #     )
        # )
        ...
        a = [list(itertools.chain.from_iterable(y))  for y in [x for x in list(zip(times, validate_times, test_times))]]
        b = [list(itertools.chain.from_iterable(y))  for y in [x for x in list(zip(lats, validate_lats, test_lats))]]
        c = [list(itertools.chain.from_iterable(y))  for y in [x for x in list(zip(lons, validate_lons, test_lons))]]
        d = [list(itertools.chain.from_iterable(y))  for y in [x for x in list(zip(delta_lats, validate_delta_lats, test_delta_lats))]]
        e = [list(itertools.chain.from_iterable(y))  for y in [x for x in list(zip(delta_lons, validate_delta_lats, test_delta_lons))]]
        g = [list(itertools.chain.from_iterable(y))  for y in [x for x in list(zip(locs, validate_locs, test_locs))]]

        a,b,c,d,e,g = shuffle(a,b,c,d,e,g)

        times, validate_times, test_times = self.split(a) 
        lats, validate_lats, test_lats = self.split(b) 
        lons, validate_lons, test_lons = self.split(c) 
        delta_lats, validate_delta_lats, test_delta_lats = self.split(d)  
        delta_lons, validate_delta_lons, test_delta_lons = self.split(e) 
        locs, validate_locs, test_locs = self.split(g) 

        # times, validate_times, test_times = self.split(shuffle([list(itertools.chain.from_iterable(y))  for y in [x for x in list(zip(times, validate_times, test_times))]], random_state=42)) 
        # lats, validate_lats, test_lats = self.split(shuffle([list(itertools.chain.from_iterable(y))  for y in [x for x in list(zip(lats, validate_lats, test_lats))]], random_state=42)) 
        # lons, validate_lons, test_lons = self.split(shuffle([list(itertools.chain.from_iterable(y))  for y in [x for x in list(zip(lons, validate_lons, test_lons))]], random_state=42)) 
        # delta_lats, validate_delta_lats, test_delta_lats = self.split(shuffle([list(itertools.chain.from_iterable(y))  for y in [x for x in list(zip(delta_lats, validate_delta_lats, test_delta_lats))]], random_state=42)) 
        # delta_lons, validate_delta_lats, test_delta_lons = self.split(shuffle([list(itertools.chain.from_iterable(y))  for y in [x for x in list(zip(delta_lons, validate_delta_lats, test_delta_lons))]], random_state=42)) 
        # locs, validate_locs, test_locs = self.split(shuffle([list(itertools.chain.from_iterable(y))  for y in [x for x in list(zip(locs, validate_locs, test_locs))]], random_state=42)) 
